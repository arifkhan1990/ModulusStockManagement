...
export const createProduct = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (req.body.locationIds && req.body.locationIds.length > 0) {
      const bulkInventoryOps = req.body.locationIds.map((locationId: string) => ({
        insertOne: {
          document: {
            companyId,
...
export const getProducts = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    res.status(200).json(successResponse('Products retrieved successfully', {
      products,
      pagination: {
        total,
        page,
...
export const getProduct = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    res.status(200).json(successResponse('Product retrieved successfully', {
      product,
...
export const deleteProduct = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (inventoryCount > 0) {
      return next(new AppError('Cannot delete product with inventory. Please update status to discontinued instead.', 400));
    }
...
export const getCategories = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    res.status(200).json(successResponse('Categories retrieved successfully', 
...
export const importProducts = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (existingProducts.length > 0) {
      ...
      return next(new AppError(`The following SKUs already exist: ${existingSkus.join(', ')}`, 409));
    }
    ...
    res.status(201).json(successResponse('Products imported successfully', {
      count: result.length,
      products: result
    }));
  } catch (error) {
